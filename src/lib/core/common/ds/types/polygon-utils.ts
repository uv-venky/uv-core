 ...
 */
export function validateCoordinatePair(pair: unknown): [number, number] {
 ...
 */
export function validatePolygonCoordinates(coordinates: unknown): [number, number][] {
  ...
  return coordinates.map((pair, index) => {
    try {
      ...
      throw new UserError(
        `Invalid coordinate pair at index ${index}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  });
}
 ...
 */
export function coordinatesToPolygonString(coordinates: [number, number][]): string {
 ...
 * Format: ((lat1,lng1),(lat2,lng2),...)
 */
export function polygonStringToCoordinates(polygonStr: string): [number, number][] {
  ...
  const coordinates: [number, number][] = matches.map((match) => {
