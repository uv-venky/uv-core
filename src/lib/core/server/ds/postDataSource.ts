...
import type { DBRow, NewRow, Row } from '@/lib/core/common/ds/types/filter';
...
export async function postDataSource<T extends object>(
  ...
  if (ds.publishSSE && !options?.skipSSE) {
    ...
    await publishSSE(client, `data:${ds.id}` as DataChannel, {
      action,
      id: ds.id,
...
async function validateBeforePost<T extends object>(
  _client: PgPoolClient,
  _session: Session,
  ds: DataSource<T>,
  rows: Row<T>[],
): Promise<{ collect?: Row<T>[]; result?: QueryResult<T> }> {
  normalizeWhitespaceForPost(ds, rows);
  validateAttributeConstraintsForPost(ds, rows);
  return { collect: rows };
}

async function groupByAndPost<T extends object>(
  client: PgPoolClient,
  session: Session,
  ds: DataSource<T>,
  rows: Row<T>[],
...
async function insertMany<T extends object>(
  ...
  let rows: DBRow<T>[] = insertedRows as unknown as DBRow<T>[];
  ...
  if (ds.type === 'Table') {
    ...
    let returnRows: DBRow<T>[] = [];
    for (let i = 0; i < len; i++) {
      const row = rows[i] as unknown as DBRow<T>;
      row._status = 'Q';
...
WHERE t.rule_id = i.rule_id;
    ...
    try {
      await client.query<DBRow<T>>(sql, params);
    } catch (e) {
      logger.error(getErrorMessage(e));
      logger.error('sql: ', sql);
      logger.error(`params: ${JSON.stringify(params)}`);
...
async function prepareForUpdate<T extends object>(
  client: PgPoolClient,
  session: Session,
  ds: DataSource<T>,
  rows: DBRow<T>[],
  attributesInUpdate: Set<string> | null,
): Promise<DBRow<T>[] | undefined> {
  ...
  if (!ds.skipQueryForUpdate && rows.length > 0) {
    ...
    const currentRows: DBRow<T>[] = [];
    const allAuditRows: NewRow<Audit>[] = [];
    for (const row of rows) {
    ...
    if (allAuditRows.length > 0) {
      await postDataSource(client, session, getDataSource<Audit>('Audit'), allAuditRows);
    }
    return currentRows;
  }
  return undefined;
