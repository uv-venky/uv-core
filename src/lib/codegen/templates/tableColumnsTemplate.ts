...
export function generateTableColumns(state: CodegenState): string {
  ...
  const filteredCols = orderedCols.filter(
    (c) =>
      !['created_by', 'last_updated_by', 'creation_date', 'last_update_date', 'created_at', 'updated_at', 'updated_by'].includes(
        c.name,
      ) &&
