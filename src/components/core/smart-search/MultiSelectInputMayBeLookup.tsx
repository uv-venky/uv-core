...
function MultiSelectInputLookup<T extends object>(props: Props & { column: SelectLookupColumn<T> }) {
  ...
  return <MultiSelectInput column={column} {...restProps} />;
}

export function MultiSelectInputMayBeLookup<T extends object>({
  column,
  ...rest
}: Props & { column: SelectColumn<T, any> }) {
  if (isSelectOptionsColumn(column)) {
    return <MultiSelectInput column={column} {...rest} />;
  }
  if (isSelectLookupColumn(column)) {
    return <MultiSelectInputLookup column={column} {...rest} />;
  }
