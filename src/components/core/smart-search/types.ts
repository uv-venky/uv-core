...
export function isSelectLookupColumn(column: SelectColumn<any, any>): column is SelectLookupColumn<any> {
  return column.type === 'Select' && 'lookupType' in column;
}

export function isSelectOptionsColumn(column: SelectColumn<any, any>): column is SelectOptionsColumn<any, any> {
  return column.type === 'Select' && !('lookupType' in column);
}

export type Column<T extends object, O extends object = any> =
  | TextColumn<T>
  | TextArrayColumn<T, O>
  | NumberColumn<T>
  | DateColumn<T>
