...
export function MultiNumberInput<T extends object>(props: Props<T>) {
    ...
    <div className="flex flex-1 flex-wrap items-center gap-1 divide-x bg-paper pl-1">
      {values.map((val, index) => (
        <div
          ...
          data-testid="multi-number-value"
        >
          {val}
          <Button
            className="h-6 w-6"
            ...
            variant="ghost"
            size="icon"
            data-testid="multi-number-remove"
          >
            <X />
          </Button>
        </div>
      ...
      {props.value.length > 1 && !expanded && (
        <Button
          ...
          variant="ghost"
          size="icon"
          data-testid="multi-number-show-all"
        >
      ...
      <Input
        ref={props.ref}
        className={cn(
          'flex-1 rounded-none border-none p-0 [appearance:textfield] placeholder:font-normal placeholder:text-secondary placeholder:text-sm focus-visible:border-none focus-visible:shadow-none focus-visible:outline-none focus-visible:ring-0 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none',
          props.className,
        )}
        style={{ lineHeight: '20px', width: '80px' }}
        type="number"
        data-testid="multi-number-input"
        onKeyDown={(e) => {
