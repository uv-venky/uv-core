...
import React, { type CSSProperties, memo, type MouseEvent } from 'react';
...
function SimplePivotTableFooterCell<ColumnKey extends string, Item>({
  columnIndex,
  data,
  isScrolling,
  rowIndex,
  ...
  const valuesBeforeCols =
    !valuesInRows && pivot.config.columnsBeforeValues !== false && colKeys.length > 0 && hasMultiMetrics;
  ...
  const seqEntry =
    useSeqLayout && data.startColumnIndex > 0 && scrollableIndex < seq.length ? seq[scrollableIndex] : undefined;
  ...
  const calculatedColumn =
    hasCalculatedColumns && data.startColumnIndex > 0 && !isFooterValuesInRows
  ...
  if (isFooterValuesInRows) {
    ...
    if (data.startColumnIndex === 0) {
      ...
      if (isValueMetric) {
        ...
        let allRecords: Array<Readonly<Item>> = [];
        if (colKey) {
        ...
        if (allRecords.length > 0 && calcCol?.formula.type === 'aggregation') {
          const result = computeCalculatedColumn(
            calcCol?.formula,
            allRecords,
            pivot.config.getNumberValue,
            pivot.config.getTextValue,
          );
          calculatedValue =
  ...
  let label: React.ReactNode = typeof value === 'string' ? value : value?.length === 1 ? value[0] : '';
  if (SPANS.includes(label as string)) {
    label = '';
    ...
    label = (
      <div className={cn(styles.wrapper, fontStyles[density], grayedOutSummaryCells && summary && styles.overlay)}>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                grayedOutSummaryCells && idx % 2 !== 0 && styles.overlay,
              )}
              style={{
    ...
    label = (
      <div className={cn(styles.wrapper, fontStyles[density])}>
        <div className={cn(styles.outer, grayedOutSummaryCells && summary && styles.overlay)}>
          <span
            className={cn(
              styles.inner,
              paddingStyles[density],
              col.alignment == null && data.startColumnIndex > 0 && !isValuesColumnForCol && alignmentStyles.end,
              col.alignment === 'center' && alignmentStyles.center,
              col.alignment === 'end' && alignmentStyles.end,
              (col.alignment === 'start' || isValuesColumnForCol) && alignmentStyles.start,
            )}
          >
            {label}
          </span>
  ...
  const cell = <div className="h-full w-full font-medium text-muted-foreground">{label}</div>;

  const onClick =
    data.startColumnIndex !== 0 && onFooterCellClick && label !== ''
