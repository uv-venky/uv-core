...
import type { Action, DispatchFn, PivotState } from '@/components/core/pivot/PivotTypes';
import { makeKey } from '@/components/core/pivot/PivotUtils';

function reducer<TColumnKey extends string, TItem>(
  state: PivotState<TColumnKey, TItem>,
  action: Action<TColumnKey, TItem>,
  ...
  switch (action.type) {
    case 'collapseRow': {
      ...
      collapseRow({
        ...
        rows,
        pivot: (pivot ?? undefined) as unknown as PivotData<string, unknown> | undefined,
      });
    ...
    case 'expandRow': {
      ...
      const makeKeyFor = (r: number) => makeKey({ rowIndex: r, columnIndex });
    ...
    case 'setPivot': {
      ...
      newState.totalRows = rowTree.totalVisible();
      newState.rowCollapseState = newRowCollapseState;
      (newState as { pivot: PivotData<TColumnKey, TItem> | null }).pivot = pivot;
      return newState;
    }
...
export default function usePivotReducer<TColumnKey extends string, TItem>(): [
  ...
  return useReducer(
    reducer as Reducer<PivotState<TColumnKey, TItem>, Action<TColumnKey, TItem>>,
    INITIAL_STATE<TColumnKey, TItem>(),
  ) as [PivotState<TColumnKey, TItem>, DispatchFn<TColumnKey, TItem>];
}
