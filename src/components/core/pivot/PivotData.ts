 ...
 * config of rows/cols.
 */
class PivotData<TColumnKey extends string, TItem> {
  ...
  constructor(inputProps: Props<TColumnKey, TItem>) {
    ...
    this.aggregator = () => {
      ...
      } satisfies Aggregator<Readonly<TItem>>;
    };
    this.tree = {};
    this.recordsTree = {};
    this.rowKeys = [];
  ...
  init(inputProps: Props<TColumnKey, TItem>): void {
    ...
    if (filters) {
      const filterKVs: Array<[TColumnKey, string[]]> = [];
      for (const [key, value] of entries(filters)) {
        if (value && value.length > 0) {
          filterKVs.push([key as TColumnKey, value]);
        }
      }
      if (filterKVs.length) {
  ...
  getIndicesToCollapseAllRows(): Array<[number, number]> {
    const removalIndices: Array<[number, number]> = [];
  ...
  getHeaderValuesBeforeCols(
    colKeys: Array<Array<string>>,
    values: Array<TColumnKey>,
    calcCols: Array<CalculatedColumn<TColumnKey>>,
    len: number,
    ...
    const topRow: Array<string> = [...this.config.rows];
    ...
    for (let i = 0; i < len; i++) {
      const headerRow: Array<string> = this.config.rows.map(() => ROW_SPAN);
  ...
  getHeaderColsBeforeValues(
    ...
    for (let i = 0; i < len; i++) {
      const headerRow: Array<string> = this.config.rows.map((r) => (i > 0 ? ROW_SPAN : r));
    ...
    const valueRow: Array<string> = this.config.rows.map(() => ROW_SPAN);
   ...
   */
  getDataColumnSequence(): Array<{ colKeyIdx: number; valueIdx?: number; calcIdx?: number }> {
    const colKeys = this.getColKeys();
    ...
    const seq: Array<{ colKeyIdx: number; valueIdx?: number; calcIdx?: number }> = [];
  ...
  getAllRecordsForCalculatedColumn(): Array<Readonly<TItem>> {
    const allRecords: Array<Readonly<TItem>> = [];
  ...
  processRecord = (record: Readonly<TItem>): void => {
    ...
    const colKey: Array<string> = [];
    const rowKey: Array<string> = [];
    const measure = this.config.measure;
    ...
    let measureValue: string | undefined;
    if (measure != null) {
  ...
  getAggregator = (rowKey: Array<string>, colKey: Array<string>): Aggregator<Readonly<TItem>> => {
    let agg: Aggregator<Readonly<TItem>> | undefined;
  ...
  getHeader(): ReadonlyArray<ReadonlyArray<string>> {
    ...
    for (let i = 0; i < len; i++) {
      const headerRow: Array<string> = this.config.rows.map((r) => {
        if (i > 0) {
          return ROW_SPAN;
        }
        return r;
  ...
  getCellData(rowIndex: number, columnIndex: number): Partial<Record<TColumnKey, string>> {
    const data: Partial<Record<TColumnKey, string>> = {};
    if (rowIndex !== -1) {
  ...
  private formatCalcResult(
    calcResult: number,
    hideZeroValues: boolean,
    formula: { mathOperator?: string; multiplier?: number },
  ): string {
  ...
  private computeCalcForRecords(
    calcCol: CalculatedColumn<TColumnKey> | undefined,
    records: Array<Readonly<TItem>>,
    hideZeroValues: boolean,
  ): string {
  ...
  private resolvedTotalLabel(): string {
    let label = totalLabel(this.config.aggregatorName);
    if (this.config.getTotalLabel) {
      label = this.config.getTotalLabel({
        aggregatorName: this.config.aggregatorName,
  ...
  getFooter(): Array<Array<string | Array<string>>> {
    ...
    const fmtAggValue = (vals: Array<number>, idx: number, agg: { format: (x: number, i?: number) => string }) => {
      const v = vals[idx];
      return v === 0 && hideZeroValues ? '' : agg.format(v, idx);
    };

    if (valuesInRows) {
      const rows: Array<Array<string | Array<string>>> = [];
      ...
      for (let m = 0; m < metricCount; m++) {
        ...
        const row: Array<string | Array<string>> = [];
  ...
  getTableData(): Array<Array<string | Array<string>>> {
    ...
    const result: Array<Array<string | Array<string>>> = [];
    let prevRow: Array<string | Array<string>> = [];
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (valuesInRows) {
        ...
        for (let metricIdx = 0; metricIdx < rowMetricCount; metricIdx++) {
          ...
          const thisRow: Array<string | Array<string>> = [];
          const row: Array<string | Array<string>> = [];
        ...
        if (this.config.cols.length) {
          ...
          if (hasCombinedTotalsBlock) {
            ...
            if (calcCols.length > 0) {
              const allRecordsForTotals: Array<Readonly<TItem>> = [];
              for (let k = 0; k < colKeys.length; k++) {
                const c = colKeys[k];
                const flatColKey = c
                  .filter((v) => !SPANS.includes(v) && v !== COL_VALUE_SPAN)
...
function forEachRecord<TItem>(
  input: ReadonlyArray<Readonly<TItem>>,
  processRecord: (record: Readonly<TItem>) => void,
): void {
