 ...
 */
function expandCell(val: Cell): Array<string> {
 ...
 */
export function buildPivotCsv<TColumnKey extends string, TItem>({
  pivot,
  columns,
  rowTree,
  columnTree,
}: BuildPivotCsvParams<TColumnKey, TItem>): string {
  ...
  const expandRowCells = (origRow: ReadonlyArray<Cell>): Array<string> => {
  ...
  const visibleBody: Array<Array<string>> = [];
  ...
  const visibleFooter: Array<Array<string>> = [];
  for (const footerRow of footer) {
    visibleFooter.push(expandRowCells(footerRow));
  }

  const allRows = [...visibleHeader, ...visibleBody, ...visibleFooter].map((row) =>
    row.map((value) => (SPANS.includes(value) ? '' : value)),
  );
