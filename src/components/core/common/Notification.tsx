...
export default function CloseAll({ toastId }: { toastId: string }) {
  const { notifications } = useSnapshot(state);
  const lastToastId = Object.keys(notifications).at(-3);
  return (
    <Button
      variant="outline"
      className={cn('absolute -top-4 right-4 z-50', lastToastId === toastId ? 'visible' : 'invisible')}
...
type State = {
  notifications: Record<string, { title: string; description?: string; count: number }>;
};
...
export function showMessage(args: ShowMsgProps): number | string {
  ...
  const a = action ?? <CloseAll toastId={id} />;
  switch (type) {
    case 'error':
...
export function removeNotification(id: string | number, onCleanup = false) {
...
export function showError(title: string | Error, props?: Omit<ShowMsgProps, 'title' | 'type'>): number | string {
  let message = '';
...
export function showWarning(title: string, props?: Omit<ShowMsgProps, 'title' | 'type'>): number | string {
  return showMessage({ ...props, title, type: 'warning' });
}

export function showInfo(title: string, props?: Omit<ShowMsgProps, 'title' | 'type'>): number | string {
  return showMessage({ ...props, title, type: 'info' });
}

export function showSuccess(title: string, props?: Omit<ShowMsgProps, 'title' | 'type'>): number | string {
