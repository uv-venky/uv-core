...
export const hasPropTillRoot = (target: HTMLElement, attr: keyof HTMLElement): boolean => {
...
export function clickOnEnterOrSpace(
  ...
  if (
    ...
    hasPropTillRoot(target as HTMLElement, 'contentEditable')
  ) {
    return;
  }
...
export default function useMove(name: string, fn: Fn) {
  ...
  const handleMouseMove = useAnimationFrame(
