...
import { PREFIX } from '@/lib/common/constants';
import { getConfig } from '@/lib/core/server/config';
import { publishSSE } from '@/lib/sse/server/index';
import { authorizeCommentAccess } from './comment-authorizer';
import { UserError } from '@/lib/core/common/error';

const LIMIT = 50;

async function requireCommentAccess(context: string, contextId: string, session: Session): Promise<void> {
...
WHERE c.context = $1 and c.context_id = $2 AND c.app_id = $3`;
  ...
  const data = await client.query<CommentWithParent>(sql, params);
...
export const createComment = async (
  ...
  await client.query<Comment>(sql, [
    _comment.id,
    context,
    contextId,
    _comment.comment,
...
export const reactToComment = async (
  ...
  if (reaction == null) {
    ...
    await client.query<Comment>(sql, [userName, commentId, appId]);
  } else {
    const sql = `UPDATE ${PREFIX}comments SET reactions[$1] = $2 WHERE id = $3 AND app_id = $4`;
    await client.query<Comment>(sql, [userName, JSON.stringify(reaction), commentId, appId]);
  }

  // Fetch updated reactions to publish
  const fetchSql = `SELECT reactions FROM ${PREFIX}comments WHERE id = $1 AND app_id = $2`;
  const result = await client.query<{ reactions: Record<string, string> }>(fetchSql, [commentId, appId]);
...
export const getCommentStats = async (
  ...
  context: string,
  contextId: string,
): Promise<{ totalComments: number; authors: string[] }> => {
  await requireCommentAccess(context, contextId, session);
  ...
  const data = await client.query<{ totalComments: number; authors: string[] }>(sql, [context, contextId, appId]);
...
export const getCommentView = async (
  ...
  const data = await client.query<{ viewedAt: Date }>(sql, [context, contextId, session.user.userName, appId]);
...
export const genID = async (_client: PgPoolClient, _session: Session): Promise<string> => {
  return ulid();
};
