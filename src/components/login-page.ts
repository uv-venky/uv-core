...
export function renderLoginPage(options: LoginPageOptions = {}): string {
  ...
  const googleAuthEnabled = options.googleAuthEnabled ?? false;
  const googleAuthUrl = options.googleAuthUrl ?? '/api/auth/google';
  const ssoEnabled = options.ssoEnabled ?? false;
  const ssoUrl = options.ssoUrl ?? '/api/auth/sso';
  const ssoButtonText = options.ssoButtonText ?? 'Sign In';
  const ssoDescription = options.ssoDescription ?? 'Only accessible via Metro One email addresses.';
  const termsUrl = options.termsUrl ?? '#';
  const privacyUrl = options.privacyUrl ?? '#';

  const showTabs = googleAuthEnabled || ssoEnabled;

  const tabListHtml = showTabs ? `
    <div class="tabs-list">
      <button type="button" class="tab-trigger active" data-tab="client">Client & Affiliate</button>
      <button type="button" class="tab-trigger" data-tab="employee">Employee Login</button>
    </div>
  ` : '';

  const employeeTabHtml = showTabs ? `
    <div id="tab-employee" class="tab-content hidden">
      <div class="sso-container">
        ${ssoEnabled ? `
          <h2 class="sso-title">${escapeHtml(brandName)} SSO Login</h2>
          <p class="sso-desc">${escapeHtml(ssoDescription)}</p>
          <button type="button" id="sso-btn" class="btn btn-primary btn-sso">${escapeHtml(ssoButtonText)}</button>
        ` : ''}
        ...
        ${googleAuthEnabled ? `
          <button type="button" id="google-btn" class="btn btn-outline btn-google">
  ...
  const clientTabStart = showTabs ? `<div id="tab-client" class="tab-content">` : '';
  const clientTabEnd = showTabs ? `</div>` : '';
...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* Background image and blur fallback */
    .bg-container {
      position: fixed;
      top: 0;
      left: 0;
...
<body>
        ...
        ${clientTabStart}
          ...
          <form id="login-form">
            <div class="form-group">
              ...
              <input id="email" name="email" type="email" autocomplete="username" class="form-control" required />
            </div>
            
            <div class="form-group">
              <label for="password" class="form-label">Password</label>
              <input id="password" name="password" type="password" autocomplete="current-password" minlength="5" class="form-control" required />
              <div class="trouble-link-container">
                <a href="/login/reset-password" class="text-link">Trouble logging in?</a>
              </div>
            </div>
            ...
            <p class="terms-text">
              ...
              and
              <a href="${escapeHtml(privacyUrl)}">Data Privacy Notice</a>.
            </p>

            <button type="submit" id="submit-btn" class="btn btn-primary" style="margin-top: 1.5rem;">Sign In</button>
          </form>
        ${clientTabEnd}
  ...
  <script>
    (function () {
      ...
      // Read sourceUrl from searchParams or fallback to default
      var sourceUrl = searchParams.get('sourceUrl') || defaultRedirect;

      // Handle displaying initialization errors if ?error= is present
      ...
      // Handle tab switching if tabs are visible
