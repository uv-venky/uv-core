...
export async function getUserByName(userName: string): Promise<DbUserRow | null> {
  const result = await executeQuery<DbUserRow>(
    `SELECT ${userSelect}
     FROM ${USERS_TABLE}
     WHERE user_name = $1`,
...
export async function getAuthUserByName(userName: string): Promise<AuthUser | null> {
