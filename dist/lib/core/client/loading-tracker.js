...
/** Max source strings embedded in data-wayvo-data-load-meta JSON (keeps DOM attr small). */
...
export function incrementPending(source) {
    ...
    if (typeof window !== 'undefined') {
        window.__WAYVO_LOADING_COUNT__ = loadingTracker.pendingCount;
    }
}
/**
...
export function resetLoadingTracker() {
    ...
    if (typeof window !== 'undefined') {
        window.__WAYVO_DATA_READY__ = false;
        window.__WAYVO_LOADING_COUNT__ = 0;
        delete document.body.dataset.storesLoaded;
        delete document.body.dataset.wayvoDataLoadMeta;
    }
}
function checkReady() {
...
function updateDOM() {
    if (typeof window === 'undefined')
        return;
    window.__WAYVO_DATA_READY__ = loadingTracker.isReady;
    if (loadingTracker.isReady) {
        document.body.dataset.storesLoaded = 'true';
        // Set metadata for Playwright tests (times are ms since mount)
        try {
            document.body.dataset.wayvoDataLoadMeta = JSON.stringify({
            ...
            if (process.env.NODE_ENV === 'development') {
                console.warn('[wayvo loading-tracker] wayvoDataLoadMeta JSON.stringify failed; using minimal fallback', e instanceof Error ? e.message : e, {
