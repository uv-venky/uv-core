...
import { applyAuditValueToRow, classifyUpdateAttributes, getPkValueStr, getWhoAttributes, loadCurrentRowsForUpdate, normalizeWhitespaceForPost, populateDefaultValues, populateWHOColumnsForUpdate, toIsoString, validateAttributeConstraintsForPost, validateRowForUpdateOrDelete, formatVectorForPostgres, } from '../../../../lib/core/server/ds/dsUtils';
            ...
            for (const attr of selectableAttributes) {
                ...
                if (val != null && attr.type === 'Polygon') {
                    if (Array.isArray(val)) {
                        ...
                        throw new UserError(`Invalid Request: Expected an array of coordinate pairs for attribute "${attr.name}", but found "${val}"`);
                    }
                }
                if (val != null && attr.type === 'Vector') {
                    val = formatVectorForPostgres(val);
    ...
    if (!ds.skipQueryForUpdate && rows.length > 0) {
        ...
        for (const row of rows) {
            ...
            if (dbUpdatedAt !== updatedAt) {
                throw new UserError(`Record has been updated by ${dbUpdatedBy} since it was read! Last updated at ${dbUpdatedAt}! You read it at ${updatedAt ?? 'null'}! (pk: ${pkValueStr})`);
            }
