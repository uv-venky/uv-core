import { isBooleanType, isDateType, isNumberType, isStringType, isTFType, isYNType, unwrapFilter, } from '../../../../lib/core/common/ds/types/attribute-utils';
import { coordinatesToPolygonString, validatePolygonCoordinates, ensurePolygonClosed, } from '../../../../lib/core/common/ds/types/polygon-utils';
import { isBooleanFilterOperator, isDateFilterOperator, isMultiDateFilterOperator, isMultiNumberFilterOperator, isMultiStringFilterOperator, isNumberFilterOperator, isStringFilterOperator, isUUIDFilterOperator, isMultiUUIDFilterOperator, isTextArrayFilterOperator, isYNFilterOperator, } from '../../../../lib/core/common/ds/types/filter';
...
export class QueryBuilder {
    ...
    validateJoinFormatConsistency() {
        ...
        const joinAliases = hasJoinsArray && this.dataSource.joins ? new Set(this.dataSource.joins.map((j) => j.alias)) : new Set();
            ...
            if (hasJoinsArray) {
                ...
                if (hasJoinAlias && attr.joinAlias) {
                    ...
                    if (!joinAliases.has(attr.joinAlias)) {
                        throw new UserError(`Attribute '${attr.code}' uses joinAlias '${attr.joinAlias}' but no join definition with that alias exists in the joins array. ` +
                    ...
                    if (hasRefTableName || hasRefWhereClause) {
                        throw new UserError(`Attribute '${attr.code}' uses joinAlias '${attr.joinAlias}' (new format) but also has refTableName/refWhereClause (old format). ` +
                ...
                if (attr.type === 'Reference' && hasRefTableName && !hasJoinAlias) {
                    throw new UserError(`DataSource '${this.dataSource.id}' uses the new joins array format, but attribute '${attr.code}' uses old format (refTableName/refWhereClause). ` +
                ...
                if (hasRefAlias && attr.refAlias && joinAliases.has(attr.refAlias) && !hasJoinAlias) {
                    throw new UserError(`Attribute '${attr.code}' uses refAlias '${attr.refAlias}' but a join with that alias exists in the joins array. ` +
                ...
                if (hasJoinAlias && attr.joinAlias) {
                    throw new UserError(`Attribute '${attr.code}' uses joinAlias '${attr.joinAlias}' (new format) but DataSource '${this.dataSource.id}' does not have a joins array. ` +
    ...
    build() {
        ...
        if (this.treeOptions && !this.treeOptions.lazyLoad) {
            ...
            recursiveSQL.push(`EXISTS (SELECT 1 FROM ${this.dataSource.schema ? `${this.dataSource.schema}.` : ''}"${this.dataSource.tableName}" AS child\n WHERE `);
    ...
    applyQuery(query) {
            ...
            query.select.forEach((attr) => {
                ...
                if (dotIndex !== -1) {
                    ...
                    if (!safeJsonKeyPattern.test(jsonKey)) {
                        throw new UserError(`Invalid select key: "${attrStr}". JSON key must be a simple identifier (letters, numbers, underscore).`);
                    }
                    ...
                    if (jsonAttribute.type !== 'JSON') {
                        throw new UserError(`Select by key "${attrStr}" is only supported for JSON attributes. "${baseAttr}" is not a JSON attribute.`);
                    }
        ...
        if (query.subSQL != null) {
            ...
            if (this.dbType === 'Postgres') {
                this.fromClause.push(`${this.dataSource.schema ? `${this.dataSource.schema}.` : ''}"${this.dataSource.tableName}" AS x`);
            }
            else {
        ...
        const queryFilters = query.filters != null && query.filter != null
        ...
        const queryMatch = query.match != null && query.data != null ? { ...query.data, ...query.match } : (query.match ?? query.data);
        if (queryMatch != null) {
        ...
        const appIdAttribute = this.dataSource.attributes.find((attr) => attr.defaultValue?.startsWith('APP_ID') && attr.column != null && attr.query !== false);
        if (appIdAttribute) {
        ...
        const schedulerIdAttribute = this.dataSource.attributes.find((attr) => attr.defaultValue === 'SCHEDULER_ID' && attr.column != null && attr.query !== false);
        if (schedulerIdAttribute) {
            const schedulerId = getConfig('applyQuery').schedulerId;
            if (logger.debugEnabled) {
                logger.debug(`Adding SCHEDULER_ID filter: ${schedulerIdAttribute.code} (${schedulerIdAttribute.column}) = ${schedulerId}`);
            }
            this.appendToWhereClause(schedulerIdAttribute.code, schedulerId);
        }
        if (query.sort != null) {
            if (query.orderBy) {
                throw new UserError(`You cannot provide both sort and orderBy. Use only one. ds: ${this.dataSource.id}, sort: ${JSON.stringify(query.sort)}, orderBy: ${query.orderBy}`);
            }
            let orderBy = '';
            ...
            for (const [attr, flag] of sortList) {
                ...
                if (dotIndex !== -1) {
                    ...
                    const safeJsonKeyPattern = /^[a-zA-Z_][a-zA-Z0-9_]*$/;
                    if (!safeJsonKeyPattern.test(jsonKey)) {
                        throw new UserError(`Invalid sort key: "${attr}". JSON key must be a simple identifier (letters, numbers, underscore).`);
                    }
                    const jsonAttribute = getAttribute(this.dataSource, baseAttr);
                    if (!jsonAttribute) {
                        throw new UserError(`Attribute not found for key: ${baseAttr}`);
                    ...
                    if (jsonAttribute.type !== 'JSON') {
                        throw new UserError(`Sort by key "${attr}" is only supported for JSON attributes. "${baseAttr}" is not a JSON attribute.`);
                    }
        ...
        if (this.selectedRefAliases.length > 0) {
            ...
            if (this.dataSource.joins && this.dataSource.joins.length > 0) {
                ...
                for (const alias of sortedJoins) {
                    const attr = this.dataSource.attributes.find((a) => a.type === 'Reference' && (a.refAlias === alias || a.joinAlias === alias));
                    if (attr?.refTableName && attr.refWhereClause) {
                        const joinType = attr.refEquiJoin ? 'INNER JOIN' : 'LEFT JOIN';
                        this.fromClause.push(` ${joinType} ${attr.refTableName} AS ${alias} ON (${typeof attr.refWhereClause === 'function' ? attr.refWhereClause() : attr.refWhereClause})`);
                    }
                }
            }
        }
            ...
            if (attr?.refWhereClause) {
                ...
                while ((match = aliasPattern.exec(clause)) !== null) {
                    ...
                    if (potentialAlias !== 'x' && potentialAlias !== alias && availableAliases.has(potentialAlias)) {
                        throw new UserError(`Nested joins are not supported in the old attribute-based format. ` +
    ...
    getJoinDependencies(joinDef) {
        ...
        if (!this.dataSource.joins)
            return [];
        const onClause = typeof joinDef.on === 'function' ? joinDef.on() : joinDef.on;
        const deps = [];
        for (const other of this.dataSource.joins) {
            if (other.alias === joinDef.alias)
                continue;
    ...
    getSortedJoinsNewFormat(aliases) {
        ...
        if (sorted.length !== aliases.length) {
            ...
            throw new UserError(`Circular dependency detected in DataSource joins. Unable to resolve join order for aliases: ${unsorted.join(', ')}`);
        }
        return sorted;
    }
    /**
    ...
    appendDateValueToWhereClause(attribute, value) {
        ...
        if (value instanceof Date) {
            ...
            if (Number.isNaN(dateValue.getTime())) {
                throw new UserError(`Invalid Request: Expected a valid date string for attribute "${attribute.name}", but found "${value}"`);
            }
        }
            ...
            if (Number.isNaN(dateValue.getTime())) {
                throw new UserError(`Invalid Request: Expected a valid date string for attribute "${attribute.name}", but found "${decodedValue}"`);
            }
        }
        else {
            throw new UserError(`Invalid Request: Expected a date value for attribute "${attribute.name}", but found "${value}"`);
        }
        this.appendAnd();
    ...
    appendNumberValueToWhereClause(attribute, value) {
        ...
        if (typeof value === 'number') {
            ...
            throw new UserError(`Invalid Request: Expected a number value for attribute "${attribute.name}", but found "${value}" of type ${typeof value}`);
        }
        this.appendAnd();
        this.appendColumnNameToWhereClause(attribute);
        this.whereClause.push(' = ');
    ...
    appendBooleanValueToWhereClause(attribute, value) {
        ...
        if (typeof value === 'boolean') {
            ...
            throw new UserError(`Invalid Request: Expected a boolean value for attribute "${attribute.name}", but found "${value}"`);
        }
        this.appendAnd();
        this.appendColumnNameToWhereClause(attribute);
        this.whereClause.push(' = ');
    ...
    appendStringValueToWhereClause(attribute, value) {
        ...
        if (typeof value === 'string' || value instanceof String) {
            ...
            throw new UserError(`Invalid Request: Expected a string value for attribute "${attribute.name}", but found "${value}"`);
        }
        this.appendAnd();
    ...
    appendPolygonValueToWhereClause(attribute, value) {
        ...
        if (Array.isArray(value)) {
            ...
            throw new UserError(`Invalid Request: Expected an array of coordinate pairs for attribute "${attribute.name}", but found "${value}"`);
        }
                    ...
                    switch (attribute.ref.type) {
                        ...
                        default:
                            if (attribute.ref.type.startsWith('Enum:')) {
                                ...
                                throw new UserError(`Field type "${attribute.type} -> ${attribute.ref.type}" for field "${attribute.name}" is not searchable!`);
                            }
                    }
                }
                else {
    ...
    buildOrderByJsonKey(attribute, jsonKey, flag) {
        ...
        switch (this.dbType) {
            ...
            default:
                throw new UserError(`Sort by JSON key is not supported for database type: ${this.dbType}. Use Postgres or MySQL.`);
        }
    }
    ...
    getJsonKeyColumnExpression(attribute, jsonKey) {
        ...
        switch (this.dbType) {
            ...
            default:
                throw new UserError(`Filter by JSON key is not supported for database type: ${this.dbType}. Use Postgres or MySQL.`);
        }
    }
    bindVar(count) {
        switch (this.dbType) {
                ...
                case 'is': {
                    ...
                    if (isNumberType(attribute.type)) {
                        ...
                        if (Number.isNaN(parsed)) {
                            throw new UserError(`Invalid number ${value} for attribute ${attribute.code} in filter ${JSON.stringify(op)}`);
                        }
    ...
    addMultiUUIDFilterToWhereClause({ op, value: values, attribute, }) {
        switch (op) {
            case 'in': {
                if (values.length === 0) {
                    throw new UserError(`Empty 'In' filter passed for attribute ${attribute.name} [${this.dataSource.id}.${attribute.code}]`);
                }
                this.appendColumnNameToWhereClause(attribute);
            ...
            case 'nin': {
                if (values.length === 0) {
                    throw new UserError(`Empty 'Not In' filter passed for attribute ${attribute.name} [${this.dataSource.id}.${attribute.code}]`);
                }
                this.appendColumnNameToWhereClause(attribute);
    ...
    addDateFilterToWhereClause({ op, value, attribute, jsonKey, }) {
        if (attribute.type === 'JSON' && jsonKey == null) {
            throw new UserError(`Date filters cannot be applied to a whole JSON attribute. Use a key inside the JSON, e.g. "${attribute.code}.dateKey" with type JSON for "${attribute.code}" in DataSource ${this.dataSource.id}.`);
        }
        const appendDateColumn = () => this.appendColumnOrJsonKeyToWhereClause(attribute, jsonKey, jsonKey ? 'date' : undefined);
        switch (op) {
            case 'on': {
    ...
    addBoolFilterToWhereClause({ op, value, attribute, jsonKey, }) {
        if (attribute.type === 'JSON' && jsonKey == null) {
            throw new UserError(`Boolean filters cannot be applied to a whole JSON attribute. Use a key inside the JSON, e.g. "${attribute.code}.flagKey" with type JSON for "${attribute.code}" in DataSource ${this.dataSource.id}.`);
        }
    ...
    addNumberFilterToWhereClause({ op, value, attribute, jsonKey, }) {
        if (attribute.type === 'JSON' && jsonKey == null) {
            throw new UserError(`Number filters cannot be applied to a whole JSON attribute. Use a key inside the JSON, e.g. "${attribute.code}.numberKey" with type JSON for "${attribute.code}" in DataSource ${this.dataSource.id}.`);
        }
    ...
    addMultiStringFilterToWhereClause({ op, value: values, attribute, ignoreCase, }) {
        switch (op) {
            case 'hasall': {
                if (values.length === 0) {
                    throw new UserError(`Empty 'Has All' filter passed for attribute ${attribute.name} [${this.dataSource.id}.${attribute.code}]`);
                }
                this.whereClause.push('(');
                values.forEach((value, index) => {
                    if (index > 0) {
            ...
            case 'hasany': {
                if (values.length === 0) {
                    throw new UserError(`Empty 'Has Any' filter passed for attribute ${attribute.name} [${this.dataSource.id}.${attribute.code}]`);
                }
                this.whereClause.push('(');
                values.forEach((value, index) => {
                    if (index > 0) {
            ...
            case 'notany': {
                if (values.length === 0) {
                    throw new UserError(`Empty 'Not Any' filter passed for attribute ${attribute.name} [${this.dataSource.id}.${attribute.code}]`);
                }
                this.whereClause.push('NOT(');
                values.forEach((value, index) => {
                    if (index > 0) {
    ...
    parseSql(sqlFragment, isOrderBy) {
        ...
        while (index !== -1) {
            ...
            if (nextIndex === -1) {
                throw new UserError(`Developer Error: Invalid ${isOrderBy ? 'order by' : 'where'} clause [${sqlFragment}] for ${this.dataSource.id}`);
            }
            ...
            if (!attr) {
                logger.error(`Developer Error: Invalid attribute [${origHa}] specified in the ${isOrderBy ? 'order by' : 'where'} clause [${sqlFragment}] of ${this.dataSource.id}`);
                throw new UserError(`Developer Error: Invalid attribute [${origHa}] specified in the ${isOrderBy ? 'order by' : 'where'} clause [${sqlFragment}] of ${this.dataSource.id}`);
            }
            // Support both refAlias (old) and joinAlias (new)
    ...
    getDateExtraType(attr) {
        if (attr.startsWith('@')) {
            if (attr.startsWith('@year'))
                return [DateExtraTypes.Year, attr.slice(5)];
            if (attr.startsWith('@month'))
                return [DateExtraTypes.Month, attr.slice(6)];
            if (attr.startsWith('@qtr'))
                return [DateExtraTypes.Quarter, attr.slice(4)];
            if (attr.startsWith('@dow'))
                return [DateExtraTypes.DayOfWeek, attr.slice(4)];
            if (attr.startsWith('@dom'))
                return [DateExtraTypes.DayOfMonth, attr.slice(4)];
            if (attr.startsWith('@doy'))
                return [DateExtraTypes.DayOfYear, attr.slice(4)];
        }
        return null;
    }
    ...
    isDateFilter(unwrappedFilter) {
        ...
        return (isDateFilterOperator(op) &&
            typeof value === 'string' &&
            (['empty', 'notempty'].includes(op) || isValid(new Date(value))));
    }
    ...
    isMultiDateFilter(unwrappedFilter) {
        ...
        return (isMultiDateFilterOperator(op) &&
            Array.isArray(value) &&
