 ...
 */
/**
 * Event type identifier for SSE events in the wayvo_events notification channel
 */
...
export type Channel = WorkflowChannel | ExecutionChannel | NotificationChannel | DataChannel | CommentChannel | CustomChannel | SystemChannel | JobStatusChannel;
/**
 ...
 */
type ExtractChannelPrefix<T extends Channel> = T extends `workflow:${string}` ? 'workflow' : T extends `execution:${string}` ? 'execution' : T extends `notification:${string}` ? 'notification' : T extends `data:${string}` ? 'data' : T extends `comment:${string}:${string}` ? 'comment' : T extends `custom:${string}` ? 'custom' : T extends 'job:status' ? 'job' : T extends '_system' ? '_system' : never;
/**
