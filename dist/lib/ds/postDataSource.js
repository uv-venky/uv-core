...
export async function postDataSource(client, auth, ds, rows) {
    if (ds.type !== 'Table') {
...
async function insertMany(client, auth, ds, insertedRows) {
    if (ds.readOnly) {
    ...
    for (const row of rows) {
        ...
        for (const attr of ds.attributes) {
            if (row[attr.code] === undefined && attr.defaultValue !== undefined) {
                row[attr.code] = attr.defaultValue === 'APP_ID' ? app_id : attr.defaultValue;
            }
            // WHO columns
            if (attr.code === 'createdAt' || attr.code === 'creationDate') {
                row[attr.code] = new Date();
            }
            else if (attr.code === 'createdBy') {
                row[attr.code] = userName;
            }
    ...
    if (ds.afterInsert) {
        const sessionMock = { user: auth.user };
        return (await ds.afterInsert({ rows: returnRows, session: sessionMock, client }));
    }
    return returnRows;
}
async function updateMany(client, auth, ds, updatedRows) {
    if (ds.readOnly) {
        throw new UserError('DataSource is read only!');
    }
        ...
        if (pkAttrs.length > 0 && updatedAtAttr && updatedAtAttr.column) {
            ...
            for (const row of rows) {
                ...
                const clientUpdatedAt = row[updatedAtAttr.code] instanceof Date
                    ? row[updatedAtAttr.code].toISOString()
                    : String(row[updatedAtAttr.code]);
                if (dbUpdatedAt !== clientUpdatedAt) {
                    throw new UserError(`Record has been updated since it was read! (DataSource: ${ds.id}). Please refresh and try again.`);
                }
            }
        }
    }
    ...
    if (ds.afterUpdate) {
        const sessionMock = { user: auth.user };
        return (await ds.afterUpdate({ rows: returnRows, session: sessionMock, client }));
    }
    return returnRows;
}
async function deleteMany(client, auth, ds, deletedRows) {
    if (ds.readOnly) {
        throw new UserError('DataSource is read only!');
    }
    ...
    if (ds.beforeDelete) {
        const sessionMock = { user: auth.user };
        rows = (await ds.beforeDelete({ rows, session: sessionMock, client }));
    }
    ...
    if (ds.afterDelete) {
        const sessionMock = { user: auth.user };
        return (await ds.afterDelete({ rows: returnRows, session: sessionMock, client }));
    }
    return returnRows;
}
