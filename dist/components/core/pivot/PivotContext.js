...
export function PivotContextProvider({ children, columns, initialSettings, onSettingsChange, }) {
    ...
    const updateCalculatedColumnWidth = useCallback((calculatedColumnId, newWidth) => {
        setSettings((prev) => ({
            ...prev,
            calculatedColumns: prev.calculatedColumns?.map((c) => (c.id === calculatedColumnId ? { ...c, width: newWidth } : c)) ?? [],
        }));
    }, []);
    const getState = useCallback(() => {
        return stateRef.current;
    }, [stateRef]);
    return (_jsx(PivotSettingsSetterContext, { value: setSettings, children: _jsx(PivotDataDispatcherContext, { value: dispatch, children: _jsx(PivotColumnsContext, { value: _columns, children: _jsx(PivotUpdateColumnWidthContext, { value: updateColumnWidth, children: _jsx(PivotUpdateCalculatedColumnWidthContext, { value: updateCalculatedColumnWidth, children: _jsx(PivotSettingsContext, { value: settings, children: _jsx(PivotDataContext, { value: state.pivot, children: _jsx(PivotRowCollapseTreeContextProvider, { totalRows: state.totalRows, tree: state.rowTree, children: _jsx(PivotColumnCollapseTreeContextProvider, { totalColumns: state.totalColumns, tree: state.columnTree, children: _jsx(PivotStateGetterContext, { value: getState, children: _jsx(PivotStateContext, { value: state, children: _jsx(PivotHeaderContext, { value: state.header, children: _jsx(PivotRowsContext, { value: state.rows, children: _jsx(PivotDraftPowerSearchFiltersContext, { value: state.draftPowerSearchFilters, children: _jsx(ErrorBoundary, { showDetails: process.env.NODE_ENV === 'development', children: children }) }) }) }) }) }) }) }) }) }) }) }) }) }) }));
}
