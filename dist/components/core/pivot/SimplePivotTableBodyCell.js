...
function SimplePivotTableBodyCell(props) {
    ...
    const { CellRenderer, columns, density, dispatch, getActualColumnIndex, getActualRowIndex, getCellStyle, grayedOutSummaryCells, header, hideBodyBottomBorder, hideBorders, hideExpandCollapseIcons, isRowCollapsed, onValueCellClick, onRowCellClick, pivot, removeColumnLines, removeRowLines, } = data.data;
    ...
    if (data.startColumnIndex > 0 && (getCellStyle || CellRenderer)) {
        ...
        const valuesBeforeCols = pivot.config.valuesPosition !== 'rows' &&
    ...
    if (typeof label === 'string' && SPANS.includes(label)) {
        ...
        label = (_jsx("div", { className: cn(styles.wrapper, 'flex'), children: metricColumns.map((c, idx) => {
                return (_jsx("div", { className: cn(styles.outer, idx !== metricColumns.length - 1 ? styles.borderRight : null, grayedOutSummaryCells && idx % 2 !== 0), style: {
        ...
        label = (_jsx("div", { className: cn(styles.wrapper), children: _jsx("div", { className: cn(styles.outer), children: _jsx("span", { className: cn(styles.inner, paddingStyles[density], col.alignment == null && data.startColumnIndex > 0 && !isValuesColumn && alignmentStyles.end, col.alignment === 'center' && alignmentStyles.center, col.alignment === 'end' && alignmentStyles.end, (col.alignment === 'start' || isValuesColumn) && alignmentStyles.start), children: label }) }) }));
    }
    ...
    const onValueClick = data.startColumnIndex !== 0 && onValueCellClick && label !== ''
        ? (e) => {
            e.stopPropagation();
            const scrollableIdx = actualColumnIndex - data.startColumnIndex;
            const valuesBeforeCols = pivot.config.valuesPosition !== 'rows' &&
                ...
                ((pivot.config.values?.length ?? 0) > 1 || (pivot.config.calculatedColumns?.length ?? 0) > 0);
            const seq = valuesBeforeCols ? pivot.getDataColumnSequence() : [];
            const colKeyIdx = valuesBeforeCols && scrollableIdx < seq.length
    ...
    const onRowClick = data.startColumnIndex === 0 && onRowCellClick && typeof value === 'string' && value !== '' && !SPANS.includes(value)
        ? (e) => {
            e.stopPropagation();
    ...
    const cell = (_jsx("div", { className: cn(fontStyles[density], styles.flexGrow, 'h-full', 'overflow-hidden', (columnSummary || rowSummary) && styles.summary, className), children: label }));
    return (_jsxs(SimplePivotTableCell, { columnIndex: columnIndex, endColumnIndex: data.endColumnIndex, endRowIndex: data.endRowIndex, hideBodyBottomBorder: hideBodyBottomBorder, hideBorders: hideBorders, removeColumnLines: removeColumnLines, removeRowLines: removeRowLines, rowIndex: rowIndex, startColumnIndex: data.startColumnIndex, style: style, className: cn(className, grayedOutSummaryCells && (columnSummary || rowSummary) && styles.overlay), type: "body", value: value, onClick: onClick, dataTestId: `pivot-cell-${columnSummary || rowSummary ? 's-' : ''}${columnSummary ? 'c-' : ''}${rowSummary ? 'r-' : ''}${rowIndex}-${columnIndex + data.startColumnIndex}`, children: [showExpandCollapseIcon ? (_jsx(ExpandCollapseIcon, { dataTestId: `ec-${rowIndex}-${columnIndex}`, isExpanded: !isRowCollapsed(actualRowIndex, rowDimColumnIndex), onClick: handleExpandCollapse })) : null, CellRenderer ? (_jsx(CellRenderer, { density: density, formattedValue: value, numberValue: vals?.length === 1 ? vals[0] : vals, context: pivot.getCellData(actualRowIndex, actualColumnIndex - data.startColumnIndex), columnIndex: columnIndex, rowIndex: rowIndex, data: makePivotItemData(data), children: cell })) : (cell)] }));
}
