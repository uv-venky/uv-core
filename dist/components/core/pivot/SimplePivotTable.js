...
import { usePivotColumnsContext, usePivotDataContext, usePivotDispatcher, usePivotHeader, usePivotRows, usePivotSettingsContext, usePivotSettingsSetterContext, usePivotState, } from '../../../components/core/pivot/PivotContext';
...
export function SimplePivotTable(props) {
    ...
    const { CellRenderer, data, disableSort = false, emptyStateSubtitle = 'Use the smart search to fetch the data', emptyStateTitle = 'No pivot data available', getCellStyle, getNumberValue, getTextValue, grayedOutSummaryCells = false, hideBorders = false, hideExpandCollapseIcons = false, hideFilters = false, hideZeroValues = true, initialCollapsed = false, initialColumnCollapsed = false, initialRowCollapsed = false, maxFooterHeight, onFooterCellClick, onValueCellClick, onRowCellClick, removeColumnLines = false, removeRowLines = false, rowHeight, showColumnTotals, showRowTotals, showGrandTotal, sorters = EMPTY_OBJECT, getTotalLabel, } = props;
    ...
    const getValueLabel = useCallback((valueKey) => {
        const column = columns.find((c) => c.key === valueKey);
        return column?.label ?? valueKey;
    }, [columns]);
    const getAllowDecimals = useCallback((valueKey) => {
        const column = columns.find((c) => c.key === valueKey);
    ...
    const header = useMemo(() => {
        if (rawHeader.length !== 1 || hideExpandCollapseIcons)
            return rawHeader;
    ...
    useEffect(() => {
        const { aggregatorName, cols, measure, rows, sort, values, valueAggregators, flattenLayout, calculatedColumns, valuesPosition, columnsBeforeValues, } = settings;
        ...
        if (!_pivot ||
            ...
            if (_pivot.getColKeys().length > 500) {
                showError(`This configuration results in ${_pivot.getColKeys().length} columns, which is too many to render in the UI. Reverting to previous settings.`);
    ...
    const hideBodyBottomBorder = hideBorders && DENSITY_PROPS[density].rowHeight * (totalRows - header.length - 1) >= height;
    ...
    const valuesBeforeCols = !valuesInRows &&
    ...
    const getEffectiveWidth = useCallback((colKey, defaultWidth = DATA_COLUMN_DEFAULT_WIDTH, skipDensityShrink = false) => {
    ...
    return (_jsx(Container, { className: cn(styles.tableContainer), children: pivot &&
            pivotItemData &&
            width > 0 &&
            height > 0 &&
            (totalRows > 0 ? (_jsx(PivotMultiGrid, { BodyCell: SimplePivotTableBodyCell, data: pivotItemData, fixedColumnCount: (pivot.rowKeys[0]?.length ?? 0) + (valuesInRows ? 1 : 0), FooterCell: SimplePivotTableFooterCell, footerRows: effectiveShowColumnTotals
                    ? valuesInRows
                        ? (values?.length ?? 0) + (calculatedColumns?.length ?? 0)
                        : 1
                    : 0, getColumnWidth: getColumnWidth, HeaderCell: SimplePivotTableHeaderCell, headerRows: header.length, height: height, maxFooterHeight: maxFooterHeight, rowHeight: rowHeightValue, totalColumns: totalColumns, totalRows: totalRows, width: width }, `${density}-${rowHeightValue}`)) : (_jsx(EmptyState, { title: emptyStateTitle, description: emptyStateSubtitle, icon: "pivot" }))) }));
}
