...
import { aggregators, compareSpans, createCompositeAggregator, getSort, totalLabel, computeCalculatedColumn, SUPPRESSED_MARKER, } from '../../../components/core/pivot/PivotUtils';
...
class PivotData {
    ...
    constructor(inputProps) {
        ...
        const baseAggregatorFactory = hasPerValueAggregators && va
    ...
    getIndicesToCollapseAllRows() {
        ...
        for (let i = 0; i < this.rowKeys.length; i++) {
            const row = this.rowKeys[i];
            if (row[0] === COL_SPAN || row[0] === BOTH_SPAN)
                continue;
            const isFirstInGroup = i === 0 || row[0] !== this.rowKeys[i - 1][0];
            const hasChildren = i + 1 < this.rowKeys.length && (this.rowKeys[i + 1][0] === row[0] || this.rowKeys[i + 1][0] === COL_SPAN);
    ...
    getIndicesToCollapseAllColumns() {
        ...
        const colsBeforeVals = !valuesInRows &&
            this.config.columnsBeforeValues === false &&
            colKeys.length > 0 &&
            (colKeys.length > 1 || colKeys[0].length > 0) &&
            (this.config.values.length > 1 || calcCols.length > 0);
        ...
        for (let i = fixedCount; i < scanEnd; i++) {
            ...
            if (val === COL_SPAN || val === ROW_SPAN || val === COL_VALUE_SPAN)
                continue;
    ...
    sortKeys(sort) {
        if (!this.sorted || this.config.sort !== sort) {
            ...
            this.rowKeys.sort(this.arrSort(this.config.measure != null ? [...this.config.rows, this.config.measure] : this.config.rows, sort));
    ...
    getHeaderValuesBeforeCols(colKeys, values, calcCols, len) {
        ...
        if (this.config.measure != null)
            topRow.push(this.config.measure);
        for (let v = 0; v < values.length; v++) {
            ...
            for (let k = 1; k < colKeys.length; k++)
    ...
    formatCalcResult(calcResult, hideZeroValues, formula) {
        ...
        if (calcResult === 0 && hideZeroValues)
            return '';
        if (formula.mathOperator === '%' || formula.multiplier === 100)
    ...
    computeCalcForRecords(calcCol, records, hideZeroValues) {
        ...
        const result = computeCalculatedColumn(calcCol.formula, records, this.config.getNumberValue, this.config.getTextValue);
     ...
     */
    getFooter() {
        if (this.config.showColumnTotals === false)
            return [];
        const colKeys = this.getColKeys();
        const effectiveColKeys = colKeys.length === 0 ? [[]] : colKeys;
        const values = this.config.values;
        ...
        const valuesBeforeCols = !valuesInRows && this.config.columnsBeforeValues !== false && hasColDims && hasMultiMetrics;
        const colsBeforeValues = !valuesInRows && this.config.columnsBeforeValues === false && hasColDims && hasMultiMetrics;
            ...
            if (valuesInRows) {
                    ...
                    if (isValueMetric) {
                        ...
                        if (colKeys.length === 0 || (colKeys.length === 1 && colKeys[0].length === 0)) {
                            ...
                            if (records && records.length > 0 && calcCol?.formula.type === 'aggregation') {
                                const calcResult = computeCalculatedColumn(calcCol?.formula, records, this.config.getNumberValue, this.config.getTextValue);
                    ...
                    if (this.config.cols.length) {
                        if (isValueMetric) {
                            ...
                            if (allRecords.length > 0 && calcCol?.formula.type === 'aggregation') {
                                const calcResult = computeCalculatedColumn(calcCol?.formula, allRecords, this.config.getNumberValue, this.config.getTextValue);
                                calculatedValue = this.formatCalcResult(calcResult, hideZeroValues, calcCol.formula);
                            }
                            row.push([calculatedValue]);
                        }
                ...
                for (const { colKeyIdx, valueIdx, calcIdx } of seq) {
                    ...
                    if (valueIdx !== undefined) {
                        ...
                        const flatColKey = colKeys.length === 0 || (colKeys.length === 1 && colKeys[0].length === 0)
                            ? ''
                ...
                if (this.config.cols.length === 0 && seq.length === 0) {
                    const aggregator = this.getAggregator(r, []);
                    const vals = aggregator.values();
                    const formattedValues = vals.map((val, idx) => val === 0 && hideZeroValues ? '' : aggregator.format(val, idx));
                ...
                if (this.config.cols.length) {
                    const calcCols = this.config.calculatedColumns ?? [];
                    const hasCombinedTotalsBlock = !valuesInRows &&
                        this.config.columnsBeforeValues === false &&
                        this.config.cols.length > 0 &&
                        (values.length > 1 || calcCols.length > 0);
                    const aggregator = this.getAggregator(r, []);
                    const vals = aggregator.values();
                    const formattedValues = vals.map((value, i) => value === 0 && hideZeroValues ? '' : aggregator.format(value, i));
                    if (hasCombinedTotalsBlock) {
                        ...
                        if (calcCols.length > 0) {
                            ...
                            for (const calcCol of calcCols) {
                                ...
                                if (allRecordsForTotals.length > 0 && calcCol.formula.type === 'aggregation') {
                                    const calcResult = computeCalculatedColumn(calcCol.formula, allRecordsForTotals, this.config.getNumberValue, this.config.getTextValue);
                                    calculatedValue = this.formatCalcResult(calcResult, hideZeroValues, calcCol.formula);
                                }
                                row.push([calculatedValue]);
                            }
                        ...
                        const hasValuesBeforeColsTotals = !valuesInRows &&
                            this.config.columnsBeforeValues !== false &&
                            colKeys.length > 0 &&
                            (colKeys.length > 1 || colKeys[0].length > 0) &&
                            (values.length > 1 || calcCols.length > 0);
