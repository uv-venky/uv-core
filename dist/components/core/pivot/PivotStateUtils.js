...
function resolveRowSpan(rows, rowIndex, colIndex) {
...
export function isHeaderEqual(a, b) {
    return (a.length === b.length &&
        a.every((rowA, index) => rowA.length === b[index].length && rowA.every((cellA, cellIndex) => cellA === b[index][cellIndex])));
}
export function collapseColumn({ columnTree, rowIndex, columnIndex, header, headerCollapseState, pivot, }) {
...
export function collapseRow({ rowTree, rowIndex, columnIndex, rowCollapseState, rows, pivot, }) {
    const valuesInRows = pivot?.config?.valuesPosition === 'rows' && (pivot?.config?.values?.length ?? 0) > 1;
    const rowMetricCount = valuesInRows && pivot ? (pivot.config.values?.length ?? 0) + (pivot.config.calculatedColumns?.length ?? 0) : 1;
...
export function getUpdatedCollapsedIndices({ pivot, oldRowCollapseState, oldRows, newRows, }) {
...
//# sourceMappingURL=PivotStateUtils.js.map