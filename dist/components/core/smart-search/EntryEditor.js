...
function EntryEditor(props) {
    ...
    const value = useMemo(() => {
        ...
        if (!attr)
            return undefined;
        const ed = filter;
        if (!isSingleFilter(ed))
            return undefined;
        const [key] = keys(ed[attr]);
        // @ts-expect-error filter[attr] is ok
        return key ? ed[attr][key] : undefined;
    }, [filter, attributeCode]);
    const ignoreCase = filter && attributeCode && filter[attributeCode] && 'ignoreCase' in filter[attributeCode]
        ? filter[attributeCode].ignoreCase
        : false;
    useEffect(() => {
        if (!attributeCode || activeSection)
            return;
    ...
    return (_jsxs("div", { role: "button", ref: outerRef, className: "editor flex max-w-full content-start items-center justify-start gap-1 overflow-hidden whitespace-nowrap rounded-full bg-default text-sm ring-2 ring-ring", onClick: (e) => {
            e.stopPropagation();
        }, onKeyDown: (e) => {
            if (e.key === 'Escape') {
                e.stopPropagation();
                ...
                if (!openField && !openOperator) {
                    dispatch({ type: 'escape' });
        ...
        }, children: [_jsx(ColumnList, { stickyFilters: stickyFilters, attributeCode: attributeCode, index: index, path: path, open: openField, onOpenChange: setOpenField }), attributeCode && (_jsxs(_Fragment, { children: [_jsx(DropdownMenuField, { dataTestId: `operator-dropdown-menu`, open: openOperator, onOpenChange: setOpenOperator, options: operatorOptions, value: operator, getLabel: (o) => {
                            const Icon = OPS_ICONS[o.value] ?? FileQuestionIcon;
                            return (_jsxs(_Fragment, { children: [o.label, " ", _jsx(Icon, { className: "size-3.5" })] }));
                        }, getValue: (o) => o.value, onChange: (option) => {
                            if (!column)
                                return;
                            dispatch({
                                type: 'setOperator',
                        ...
                        }, startIcon: showMatchCase && (_jsx(Button, { variant: "ghost", size: "icon", "data-testid": "match-case-button", onClick: (e) => {
                                e.stopPropagation();
                                if (!column)
                                    return;
                                dispatch({
                            ...
                            }, className: cn('shrink-0', ignoreCase
                                ? 'text-blue-600 hover:text-blue-700'
                                : 'text-muted-foreground hover:text-muted-foreground'), "data-tip": ignoreCase ? 'Match Case' : 'Ignore Case', children: _jsx(CaseSensitive, {}) })), onCloseAutoFocus: (e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            const el = inputRef.current;
                            if (!el)
                                return;
                            el.focus();
                        } }), column && operator && (_jsx(EntryEditorValueInput, { open: openValue, onOpenChange: setOpenValue, column: column, ref: inputRef, operator: operator, onChange: (value, done) => {
                            dispatch({
                                type: 'setValue',
                                path,
                                index,
                        ...
                        }, value: value, path: path }))] })), _jsx(Button, { onClick: (e) => {
                    e.stopPropagation();
                    dispatch({ type: 'removeFilter', path: [...path, index] });
                }, variant: "ghost", size: "icon", className: "shrink-0", "data-tip": "Remove filter", "data-testid": "remove-filter-button", children: _jsx(X, {}) })] }));
}
