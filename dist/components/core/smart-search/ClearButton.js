...
export function ClearButton({ stickyFilters }) {
    ...
    return (state.filters.length > 0 &&
        state.filters.some((f) => !isStickyFilter(f, stickyFilters)) && (_jsx(Button, { variant: "ghost", size: "icon", "data-testid": "clear-button", onClick: (e) => {
            e.stopPropagation();
            dispatch({ type: 'clearSearch' });
        }, "data-tip": "Clear Filters", children: _jsx(X, {}) })));
}
