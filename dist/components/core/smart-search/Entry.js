...
function DisplayValue(props) {
    return (_jsx("div", { ref: props.valueRef, role: "button", tabIndex: props.onClick ? -1 : undefined, className: cn('val max-w-[160px] shrink overflow-hidden overflow-ellipsis px-2 focus:outline', {
            'cursor-pointer': props.onClick,
        }), onClick: props.onClick, "data-testid": props.dataTestId, children: props.column.type !== 'Select' && props.column.type !== 'TextArray' ? (Array.isArray(props.value) ? (props.value.join(', ')) : (props.value)) : (_jsx(DisplaySelectValueMayBeLookup, { ...props, column: props.column })) }));
}
function SingleEntry({ column, filter, readOnly: ro, path, stickyFilters, attributeCode, active, }) {
    ...
    return (_jsxs("div", { ref: outerRef, role: "button", tabIndex: 0, onClick: handleValueClick, onKeyDown: (e) => {
            e.stopPropagation();
            if (e.key === 'ArrowLeft') {
        ...
        }, className: cn('entry flex max-w-full select-none items-center divide-x overflow-hidden whitespace-nowrap rounded-full border bg-default px-1 text-sm ring-ring focus:ring-2', { 'cursor-default': readOnly }), style: { height: '30px' }, "data-testid": `filter-entry-${path.join('-')}-${attributeCode}`, onFocus: () => {
            dispatch({ type: 'setActivePath', path });
        }, children: [_jsx("div", { role: "button", tabIndex: !readOnly ? -1 : undefined, className: cn('attr grow-0 px-2', !readOnly && 'cursor-pointer'), onClick: !readOnly ? handleFieldClick : undefined, onKeyDown: readOnly ? undefined : (e) => clickOnEnterOrSpace(e, handleFieldClick), "data-testid": `filter-field-${path.join('-')}-${attributeCode}`, children: columnLabel }), _jsxs("div", { role: "button", tabIndex: !readOnly ? -1 : undefined, className: cn('op flex grow-0 items-center gap-2 px-2 ring-ring focus:ring-2', !readOnly && 'cursor-pointer'), onClick: !readOnly
                    ? (e) => {
                        e.stopPropagation();
                        e.preventDefault();
                        dispatch({ type: 'editPath', path, activeSection: 'operator' });
                    }
                    : undefined, "data-testid": `filter-operator-${path.join('-')}-${attributeCode}`, children: [filter[attributeCode] && 'ignoreCase' in filter[attributeCode] && filter[attributeCode].ignoreCase && (_jsx(CaseSensitive, { className: "h-4 w-4 text-blue-600", "data-tip": "Ignore Case" })), operatorLabel] }), hasEditor(operator) && (_jsx(DisplayValue, { valueRef: valueRef, dataTestId: `filter-value-${path.join('-')}-${attributeCode}`, value: displayValue, column: column, onClick: !readOnly ? handleValueClick : undefined })), !readOnly && (_jsx(Button, { tabIndex: -1, "data-testid": `filter-remove-${path.join('-')}-${attributeCode}`, onClick: (e) => {
                    e.stopPropagation();
                    dispatch({ type: 'removeFilter', path });
                }, className: "shrink-0", variant: "ghost", size: "icon", children: _jsx(X, {}) }))] }));
}
function Entry(props) {
    ...
    return ['allof', 'anyof', 'noneof'].includes(attributeCode) ? (_jsx(NestedEntry, { readOnly: readOnly, path: path, stickyFilters: stickyFilters })) : (column && (_jsx(SingleEntry, { ...props, column: column, path: path, attributeCode: attributeCode, filter: filter })));
}
