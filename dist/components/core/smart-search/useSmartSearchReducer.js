...
import { CASE_SENSITIVE_OPS, getDefaultOperator, getDefaultValue, hasEditor, isMultiOperator, } from '../../../components/core/smart-search/operators';
...
import { isSamePath } from '../../../components/core/mutX';
...
function removeEmptyNestedFilters(props) {
...
function reducer(state, action) {
    switch (action.type) {
        ...
        case 'setActiveView': {
            ...
            const newFiltersWithoutSticky = (activeView?.payload?.filters ?? EMPTY_ARRAY).filter((f) => !state.stickyFilters.includes(splitFilter(f).attributeCode));
        ...
        case 'removeFilter': {
            ...
            newState.activePath = updatePathAfterRemove(filters, newPath);
            newState.editing = false;
        ...
        case 'escape': {
            ...
            newState.activePath = updatePathAfterRemove(newState.filters, newState.activePath);
            newState.editing = false;
            return newState;
        }
        case 'setActivePath': {
...
function checkAndAdjustNestedPath(filters, path) {
...
function updatePathAfterRemove(filters, path) {
...
function moveLeft(filters, path) {
...
function moveRight(filters, path) {
    if (!hasIn(filters, path) || path.length === 0) {
        return EMPTY_ARRAY;
    }
    const lastIndex = path[path.length - 1];
...
export default function useSmartSearchReducer() {
    return useReducer(reducer, INITIAL_STATE());
}
//# sourceMappingURL=useSmartSearchReducer.js.map