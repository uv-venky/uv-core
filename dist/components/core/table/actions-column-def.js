...
export function createActionsColumn(actions, options) {
    ...
    return {
        ...
        size,
        header: (props) => {
            return (_jsx(HeaderCell, { ...props, type: "Text", store: props.table.options.meta?.store, accessorKey: ACTIONS_COLUMN_ID, title: title, align: "center" }));
        },
        cell: (props) => {
            const rowId = props.row.id;
            return (_jsx(ActionsColumnCell, { actions: actions, rowId: rowId, table: props.table, store: props.table.options.meta?.store }));
        },
    };
}
