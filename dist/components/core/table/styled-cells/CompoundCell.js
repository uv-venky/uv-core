 ...
 * ```
 */
export function CompoundCell({ primary, secondary, preset, icon, iconBgClass, iconClass, onClick, useTableOnEdit, className, feedbackMask, row, table, }) {
    ...
    const content = (_jsxs(_Fragment, { children: [resolvedIcon && (_jsx("div", { className: cn('flex size-6 shrink-0 items-center justify-center rounded', resolvedIconBgClass), children: _jsx("span", { className: resolvedIconClass, children: resolvedIcon }) })), _jsxs("div", { className: "flex min-w-0 flex-col", children: [isClickable ? (_jsx("span", { className: "truncate font-medium text-sm group-hover:underline", children: primaryString })) : (_jsx("span", { className: "truncate font-medium text-sm", children: primaryString })), secondaryString && _jsx("span", { className: "truncate text-muted-foreground text-xs", children: secondaryString })] })] }));
    return (_jsx(Cell, { attributeCode: primary, store: store, rowId: row.id, className: cn('px-2 py-1', className), feedbackMask: feedbackMask, children: isClickable ? (_jsx("button", { type: "button", onClick: handleClick, className: "group flex min-w-0 max-w-full cursor-pointer items-center gap-2 text-left", children: content })) : (_jsx("div", { className: "flex min-w-0 max-w-full items-center gap-2", children: content })) }));
}
