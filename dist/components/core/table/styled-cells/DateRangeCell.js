 ...
 * ```
 */
export function DateRangeCell({ startDateField, endDateField, variant = 'stacked', startLabel, endLabel, separator = ' – ', formatDate = defaultFormatDate, className, feedbackMask, row, }) {
    ...
    if (variant === 'inline') {
        return (_jsx(Cell, { attributeCode: startDateField, store: store, rowId: row.id, className: className, feedbackMask: feedbackMask, children: _jsx("span", { className: "text-sm", children: start && end ? (_jsxs(_Fragment, { children: [start, separator, end] })) : start ? (_jsxs(_Fragment, { children: ["From ", start] })) : (_jsxs(_Fragment, { children: ["Until ", end] })) }) }));
    }
    // Stacked variant (default): "From: Jan 1" / "To: Jan 31"
    const fromLabel = startLabel ?? 'From:';
    const toLabel = endLabel ?? 'To:';
    return (_jsx(Cell, { attributeCode: startDateField, store: store, rowId: row.id, className: className, children: _jsxs("div", { className: "flex flex-col gap-0.5 text-xs", children: [start && (_jsxs("span", { children: [_jsx("span", { className: "text-muted-foreground", children: fromLabel }), " ", start] })), end && (_jsxs("span", { children: [_jsx("span", { className: "text-muted-foreground", children: toLabel }), " ", end] }))] }) }));
}
