...
import { isWhoAttribute } from '../../../lib/core/common/ds/types/Attribute';
...
export async function validateRecords({ data, attrs, pkAttrs, dateTimeFormat, dateFormat, maxRows, defaultFunction, fileName, fileType, fileSize, }) {
    ...
    if (defaultFunction) {
        ...
        if (result.status === 'SUCCESS') {
            if (Array.isArray(result.result)) {
                ...
                throw new Error(`Developer Error: Default function ${defaultFunction} must return an array of records! Instead it returned ${JSON.stringify(result.result)}`);
            }
        }
    ...
    for (let i = 0; i < data.length; i++) {
            ...
            if (val !== undefined && val != null) {
                ...
                if (isDateType(attr.type)) {
                    ...
                    if (val instanceof Date) {
                        ...
                        throw new Error(`Invalid date value '${val}' of type '${typeof val}' for ${label} at row ${i + 1}! Must be a date cell!`);
                    }
                }
                    ...
                    if (typeof val === 'boolean') {
                        ...
                        if (val === 1 || val === 0) {
                            ...
                            throw new Error(`Invalid boolean value '${val}' for ${label} at row ${i + 1}! Must be 0 (false) or 1 (true)!`);
                        }
                    }
                        ...
                        if (lowerVal === 'true' || lowerVal === 'false') {
                            ...
                            throw new Error(`Invalid boolean value '${val}' for ${label} at row ${i + 1}! Must be "false", "true", "FALSE", or "TRUE"!`);
                        }
                    }
                    else {
                    ...
                    if (attr.maxLength && val.length > attr.maxLength) {
                        throw new Error(`Value '${val}' for ${label} at row ${i + 1} is too long ${val.length} chars! Maximum length allowed is ${attr.maxLength} chars!`);
                    }
                    record[attr.code] = val;
                }
                    ...
                    throw new Error(`Invalid value [${val}] of type '${typeof val}' for ${label} at row ${i + 1}. Expected ${attr.type}!`);
                }
            }
        ...
        if (Object.keys(row).length > 0) {
            throw new Error(`Invalid column titles ${Object.keys(row).join(', ')} at row ${i + 1}! Allowed column titles are ${attrs
                .map((attr) => attr.code)
                .join(', ')}`);
        }
        if (pkVal) {
...
//# sourceMappingURL=validateRecords.js.map