...
export default function TableFilters({ store, table, columns, pageId, itemId, border, roundedCorners, disableSavedSearches, stickyFilters, searchOnBlur = true, updateFilters, }) {
    const [filters, setFilters] = useStoreFilters(store);
    ...
    const onSearch = useCallback(({ custom, filters }, action) => {
        ...
        const shouldExecuteQuery = hasSavedFilters || action === 'search-click';
    ...
    return (_jsx(SmartSearch, { stickyFilters: stickyFilters, border: border, roundedCorners: roundedCorners, columns: columns, filters: filters, onSearch: onSearch, searchOnBlur: searchOnBlur, savedSearch: savedSearch, isBusy: isBusy }));
}
