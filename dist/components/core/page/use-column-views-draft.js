...
import { applyTablePageSize, createTableColumnPreferences, getDefaultPageSize, getDefaultTableColumnPreferences, } from '../../../components/core/page/table-column-preferences';
import { getDefaultColumnOrder, getDefaultColumnVisibility } from '../../../components/core/page/useTable';
function getHideableColumnOptions(table) {
    return table
        .getAllColumns()
...
function buildDisplayedColumnIds(table) {
...
function buildDraftFromTableState(table, preferences) {
...
export function createDraftFromTable(table, preferences) {
    return buildDraftFromTableState(table, preferences);
}
function createSnapshot(table, preferences) {
    const { columnOrder, columnVisibility } = table.getState();
    return {
...
function bumpTableUpdate(table) {
...
function applyDraftToTable(table, draft, onPreferencesChange) {
...
async function restoreSnapshot(table, snapshot, onPreferencesChange) {
...
function resolveDefaultPreferences(table, defaultPreferences) {
...
function applyDefaultColumnsState(table, defaultPreferences, keepPreferences, keepPageSize) {
    const meta = table.options.meta;
...
export function isColumnsTabDirty(baseline, draft) {
    if (!arraysEqual(baseline.displayedColumnIds, draft.displayedColumnIds))
        return true;
    const keys = new Set([...Object.keys(baseline.columnVisibility), ...Object.keys(draft.columnVisibility)]);
    for (const key of keys) {
        if (baseline.columnVisibility[key] !== draft.columnVisibility[key])
            return true;
    }
    return false;
}
export function isDensityTabDirty(baseline, draft) {
    return baseline.preferences.tableVariant !== draft.preferences.tableVariant || baseline.pageSize !== draft.pageSize;
}
export function isStickyTabDirty(baseline, draft) {
    return (baseline.preferences.stickyLeftCount !== draft.preferences.stickyLeftCount ||
        baseline.preferences.stickyRightCount !== draft.preferences.stickyRightCount);
}
export function isSessionDraftDirty(baseline, draft) {
    return isColumnsTabDirty(baseline, draft) || isDensityTabDirty(baseline, draft) || isStickyTabDirty(baseline, draft);
}
function buildDefaultDraft(table, defaultPreferences) {
    const meta = table.options.meta;
    const tableColumns = meta.tableColumns ?? [];
    const defaultVisibleColumnOrder = meta.defaultVisibleColumnOrder;
    ...
    const displayedColumnIds = defaultOrder.filter((id) => {
        ...
        if (id in defaultVisibility)
            return defaultVisibility[id] !== false;
        return true;
    });
...
export function hasPersistedTableCustomization(table, snapshot, defaultPreferences) {
    const defaultDraft = buildDefaultDraft(table, defaultPreferences);
    return isSessionDraftDirty(defaultDraft, snapshot.draft);
}
export function useColumnViewsDraft({ table, preferences, onPreferencesChange, defaultPreferences, }) {
    const [open, setOpen] = useState(false);
    const [activeTab, setActiveTab] = useState('columns');
    const [draft, setDraft] = useState(() => createDraftFromTable(table, preferences));
    const [isPageSizeBusy, setIsPageSizeBusy] = useState(false);
    const sessionStartRef = useRef(createSnapshot(table, preferences));
    ...
    const updatePageSize = useCallback(async (pageSize) => {
    ...
    const openPopover = useCallback(() => {
        const currentPreferences = table.options.meta.preferences ?? preferences;
    ...
    const handleResetTab = useCallback((tab) => {
        if (tab === 'columns') {
            setDraft((prev) => {
    ...
    const resetDisabled = !open || isPageSizeBusy || !hasPersistedTableCustomization(table, sessionStartRef.current, defaultPreferences);
...
//# sourceMappingURL=use-column-views-draft.js.map