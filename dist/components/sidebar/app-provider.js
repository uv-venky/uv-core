...
const LOADING_SHELL = (_jsxs("div", { className: "flex h-full w-full items-center justify-center gap-2 bg-background", children: [_jsx(Loader2, { className: "h-4 w-4 animate-spin" }), " Loading..."] }));
export function AppProvider({ hideSidebar, children, ...props }) {
    ...
    if (error) {
        return (_jsxs("div", { className: "flex h-full w-full flex-col items-center justify-center gap-3 p-4", children: [_jsx("div", { className: "w-full max-w-2xl [&>div]:h-auto", children: _jsx(ErrorCard, { children: error }) }), _jsx(Button, { activityId: "app-provider-session-error-sign-out", type: "button", variant: "link", className: "text-sm", onClick: async () => {
    ...
    return (_jsx(Suspense, { fallback: LOADING_SHELL, children: _jsx(BaseAppProvider, { ...props, children: _jsx(ClientRootLayout, { session: session, hideSidebar: hideSidebar, children: children }) }) }));
}
