...
import { APP_DESCRIPTION as DEFAULT_APP_DESCRIPTION, APP_NAME as DEFAULT_APP_NAME, DISABLE_HEADER_FILTERS_DEFAULT as DEFAULT_DISABLE_HEADER_FILTERS, IGNORE_CASE_DEFAULT as DEFAULT_IGNORE_CASE, TEST_PASSWORD as DEFAULT_TEST_PASSWORD, } from '../../lib/common/ui-constants';
...
export function BaseAppProvider({ children, executeSidebarAction, isSidebarItemActive = defaultIsSidebarItemActive, dynamicSidebarContent = () => null, APP_NAME = DEFAULT_APP_NAME, APP_DESCRIPTION = DEFAULT_APP_DESCRIPTION, DISABLE_HEADER_FILTERS_DEFAULT = DEFAULT_DISABLE_HEADER_FILTERS, IGNORE_CASE_DEFAULT = DEFAULT_IGNORE_CASE, TEST_PASSWORD = DEFAULT_TEST_PASSWORD, deployConfig, awsRegion, gitHubRepoName, whyDidYouUpdateEnabled = false, whyDidYouUpdateNames = [], customMiniLogo, workflowPlugins, customSidebarIcons, tableVariant, notificationProvider, }) {
    ...
    // Register workflow client plugins on mount
    useEffect(() => {
        if (workflowPlugins && workflowPlugins.length > 0) {
            for (const plugin of workflowPlugins) {
                registerIntegration(plugin.integration);
            }
            console.info(`[Workflow Plugins] Registered ${workflowPlugins.length} client plugin(s)`);
        }
    }, [workflowPlugins]);
