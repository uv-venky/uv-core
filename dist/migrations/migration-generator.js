...
function defaultVersion() {
    ...
    const suffix = String(now.getUTCHours() * 3600 + now.getUTCMinutes() * 60 + now.getUTCSeconds()).padStart(5, '0');
...
export async function createMigrationFile(options) {
    ...
    const migrationsDir = path.resolve(process.cwd(), options.migrationsDir ?? getConfig().migrationsDir);
